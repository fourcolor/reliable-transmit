...
** current time get passed a pointer to a struct expr_time.  If it's non-NULL
** it gets used, otherwise we do our own expr_time_now() to fill it in.
** This lets the caller avoid extraneous expr_time_now()s when efficiency
** is needed, and not bother with the extra code when efficiency doesn't
** matter too much.
*/
static void
getnow( struct expr_time* nowP, struct expr_time* nowP2 )
{
    if ( nowP != NULL )
	*nowP2 = *nowP;
    else
...
list_add( Timer* t )
{
	...
	if (expr_time_compare(&t->time, &timers->time) < 0) {
		...
		if (expr_time_compare(&t->time, &t2->time) < 0) {
		    /* Found it. */
		    t2prev->next = t;
		    t->prev = t2prev;
		    t->next = t2;
...
Timer*
tmr_create(
    struct expr_time* nowP, TimerProc* timer_proc, TimerClientData client_data,
    int64_t usecs, int periodic )
{
    ...
    t->periodic = periodic;
    t->time = now;
    expr_time_add_usecs(&t->time, usecs);
...
struct timeval*
tmr_timeout( struct expr_time* nowP )
{
    struct expr_time now, diff;
    int64_t usecs;
    int past;
    ...
    if ( timers == NULL )
	return NULL;
    past = expr_time_diff(&timers->time, &now, &diff);
    if (past)
        usecs = 0;
    else
        usecs = expr_time_in_usecs(&diff);
...
tmr_run( struct expr_time* nowP )
{
	...
	*/
	if (expr_time_compare(&t->time, &now) > 0)
	    break;
	(t->timer_proc)( t->client_data, &now );
	if ( t->periodic ) {
	    /* Reschedule. */
	    expr_time_add_usecs(&t->time, t->usecs);
	    list_resort( t );
	} else
	    tmr_cancel( t );
    }
...
void
tmr_reset( struct expr_time* nowP, Timer* t )
{
    struct expr_time now;
