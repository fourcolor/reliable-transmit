...
** the TimerClientData associated with the timer, and a expr_time in case
** it wants to schedule another timer.
*/
typedef void TimerProc( TimerClientData client_data, struct expr_time* nowP );
...
*/
extern struct timeval* tmr_timeout( struct expr_time* nowP ) /* __attribute__((hot)) */;

/* Run the list of timers. Your main program needs to call this every so often,
** or as indicated by tmr_timeout().
*/
extern void tmr_run( struct expr_time* nowP ) /* __attribute__((hot)) */;

/* Reset the clock on a timer, to current time plus the original timeout. */
extern void tmr_reset( struct expr_time* nowP, Timer* timer );
